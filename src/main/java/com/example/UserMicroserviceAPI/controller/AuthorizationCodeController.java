...
public class AuthorizationCodeController {
    ...
    public ResponseEntity<AuthorizationCode> createCombinedCode(@RequestBody CombinedAuthorizationRequest request) {
       ...
       // Endpoint to get all authorization codes
       @GetMapping("/")
       public ResponseEntity<List<AuthorizationCode>> getAllAuthorizationCodes() {
           List<AuthorizationCode> codes = authorizationCodeService.getAllAuthorizationCodes();
           return ResponseEntity.ok(codes);
       }
   
       // Endpoint to get authorization codes by licensee ID
       @GetMapping("/licensee/{licenseeId}")
       public ResponseEntity<Optional<AuthorizationCode>> getAuthorizationCodesByLicenseeId(@PathVariable Long licenseeId) {
        Optional<AuthorizationCode> codes = authorizationCodeService.getAuthorizationCodesByLicenseeId(licenseeId);
           return ResponseEntity.ok(codes);
       }
}
